...
- **상점(짝수층 2·4)**: 패시브 아이템 슬롯은 방 생성 시 `ITEM_DEFS` 중 1개를 미리 선정하여 카드에 이름·설명을 노출. 구매 시 `PassiveItem.grant`로 즉시 적용 + unlock 등록 (가격은 일반 패시브와 동일 60, `design/SHOP.md` 참조). 스택형은 보유 중이어도 상시 후보.
...
| `core_crystal` | 코어 결정체 | 청록 `0x00e0ff` | **기본 공격력 +1** (중복 획득 가능). 스택형 — 드롭·상점(60코어)·시작방에 일반 아이템처럼 등장 + 일반 패시브 소진 시 확정 폴백 드롭 | `player.baseAttack += 1` |
