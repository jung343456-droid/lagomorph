...
- **상점(각 구역 표시 2·4·7·9층)**: 패시브 아이템 슬롯은 방 생성 시 `ITEM_DEFS` 중 1개를 미리 선정하여 카드에 이름·설명을 노출. 구매 시 `PassiveItem.grant`로 즉시 적용 + unlock 등록 (가격은 일반 패시브와 동일 75, `design/SHOP.md` 참조). 스택형은 보유 중이어도 상시 후보.
