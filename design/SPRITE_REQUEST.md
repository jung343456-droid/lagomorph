...
- 배경: 황폐화된 숲 던전, 어두운 톤  
- 주인공: 사이버네틱 강화 토끼 **LAGO-7 (soma)** — 전투용 보호대와 바이저를 장착한 군사 개조체  
...
```
Pixel art sprite sheet, transparent background.
LAGO-7 (codename: soma), a cybernetically enhanced combat rabbit soldier (64x72 px each frame).
...
# ✦ 구역 3 — "사냥꾼" 적 캐릭터 추가 요청 (2026-06)

## 구역 3 컨텍스트

훈련장(숲)을 빠져나온 LAGO가 처음으로 **인간 사냥꾼**과 결착하는 구역. 필드는 구역 1과 비슷한 **풀숲형**
이지만, 곳곳에 **나무로 지은 집·오두막·울타리**가 놓여 있고 **동물 + 인간 사냥꾼**이 섞여 등장한다.
구역 4는 동일 로스터를 강화·랜덤 배치하므로 **신규 스프라이트는 구역 3에서 한 번만 제작**한다.

- 인간 사냥꾼 2종(단검 근접 / 활 원거리) + 동물 3종(뱀 / 까마귀 / 오소리) + 보스 2종(사냥개 무리 / 사냥꾼 보스)
- **시각적 톤**: 동물은 기존 "변이 + 붉은 눈 + 사이버 흔적" 유지. **인간 사냥꾼은 거칠고 누더기 같은
  가죽 사냥복 + 후드/마스크**, 눈만 붉게 빛나(훈련 로봇 복선) 인간형이지만 어딘가 비인간적인 인상.

> **공통 스타일 지시문(아래 모든 프롬프트에 prefix로 추가)**
>
> ```
> Pixel art sprite for a top-down roguelike game called LAGOMORPH (Zone 3 - "the hunters").
> Style: 16x16-base pixel art, top-down 45-degree quarter-view, transparent PNG background,
> limited palette (8-12 colors per character), grassy field with wooden cabins aesthetic.
> Zone 3 features human hunters (rugged leather hunting garb, hoods/masks, faintly glowing red eyes —
> hinting they are training robots) alongside wild animals with red eyes and subtle cyber implants.
> Clean pixel art, no anti-aliasing, crisp edges.
> ```
...
| 항목 | 값 |
|---|---|
| 저장 위치 | `public/assets/enemies/{name}/` — 1·2구역과 동일 |
| 디렉토리 7개 | `daggerhunter/`, `bowhunter/`, `snake/`, `crow/`, `badger/`, `hound/`, `hunterboss/` |
| 파일명 규칙 | `{name}-{state}.png`, 8방향 키 `{name}-{n,ne,e,se,s,sw,w,nw}.png` |
| 8방향 키 | `n` 위 / `ne` 우상 / `e` 우 / `se` 우하 / `s` 아래 / `sw` 좌하 / `w` 좌 / `nw` 좌상 |
| 보조 텍스처 | `snare`(올가미 덫), `hunter-arrow`(화살 투사체) — 코드 placeholder 가능, PNG 받으면 교체 |

---

## 1. 단검 사냥꾼 (DaggerHunter) — 근접 연타 추격형

- **게임 내 크기**: 40×56 px (인간형, 세로로 긴 실루엣)
- **기준 색상**: 가죽복 `#6B5436`, 후드 `#4A3A24`, 단검 날 `#C8CCD0`, 붉은 눈 `#FF3322`
- **특징**: 누더기 가죽 사냥복 + 후드, 한 손에 단검. 베기(slash) 시 단검을 크게 휘두르는 전진 자세.
- **파일 목록**: `daggerhunter-{8방향}.png`(40×56, 보행) + `daggerhunter-windup.png`(예고: 단검 뒤로 당김) + `daggerhunter-slash.png`(전진 베기)

```
Pixel art sprite sheet, transparent background.
A human DAGGER HUNTER enemy for Zone 3 of LAGOMORPH roguelike (40x56 px, tall humanoid).
Rugged leather hunting garb (#6B5436), dark hood (#4A3A24), one hand gripping a steel dagger (#C8CCD0),
faintly glowing red eyes (#FF3322) under the hood.
8-directional walking sprites in 2x4 grid (each 40x56 px): [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames on a strip: [windup 40x56] dagger drawn back, crouched ready;
[slash 40x56] lunging forward with a wide dagger swing, motion arc.
Top-down 45-degree view. 10-color limited palette.
```

---

## 2. 활 사냥꾼 (BowHunter) — 원거리 조준 사격형

- **게임 내 크기**: 40×56 px
- **기준 색상**: 가죽복 `#5A6B3A`(녹색 계열로 단검 사냥꾼과 구분), 후드 `#3A4A28`, 활 `#8B6B3A`, 붉은 눈 `#FF3322`
- **특징**: 장궁을 든 사수. 조준(aim) 시 활시위를 당긴 정지 자세. 올가미 덫도 설치하므로 허리에 밧줄 디테일 권장.
- **파일 목록**: `bowhunter-{8방향}.png`(40×56) + `bowhunter-aim.png`(시위 당김 조준) + `bowhunter-shoot.png`(발사 직후)

```
Pixel art sprite sheet, transparent background.
A human BOW HUNTER enemy for Zone 3 of LAGOMORPH roguelike (40x56 px, tall humanoid).
Green-tinted leather garb (#5A6B3A), hood (#3A4A28), holding a wooden longbow (#8B6B3A),
coils of rope at the belt (for snare traps), faintly glowing red eyes (#FF3322).
8-directional walking sprites in 2x4 grid (each 40x56 px): [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames: [aim 40x56] bowstring drawn, standing still and focused;
[shoot 40x56] arrow just released, string snapping forward.
Plus a small frame: [arrow projectile 16x6] a sharp wooden arrow, horizontal (rotated in-engine).
Top-down 45-degree view. 10-color limited palette.
```
...
- **게임 내 크기**: 40×28 px (또아리 자세 기준, 길고 낮은 실루엣)
- **기준 색상**: 비늘 `#4A7A3A`(독초록), 무늬 `#2A4A22`, 배 `#B0A060`, 붉은 눈 `#FF3322`
- **특징**: 위에서 본 또아리 튼 뱀. 잠복(lurk) 시 낮게 웅크림, 기습(strike) 시 직선으로 길게 뻗어 무는 자세. 독니 강조.
- **파일 목록**: `snake-{8방향}.png`(40×28, 이동) + `snake-lurk.png`(또아리 잠복) + `snake-strike.png`(직선 런지 물기)

```
Pixel art sprite sheet, transparent background.
A mutated SNAKE enemy for Zone 3 of LAGOMORPH roguelike (40x28 px, low coiled silhouette).
Toxic-green scales (#4A7A3A) with dark banding (#2A4A22), pale belly (#B0A060),
glowing red eyes (#FF3322), prominent venom fangs.
8-directional top-down sprites in 2x4 grid (each 40x28 px): [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames: [lurk 40x28] coiled low, hidden in grass, head tucked;
[strike 40x28] body stretched in a straight lunging bite, fangs bared.
Top-down 45-degree view. 8-color limited palette.
```

---

## 4. 까마귀 (Crow) — 공중 정찰·표식 지원형

- **게임 내 크기**: 32×26 px (날개 펼친 비행 실루엣)
- **기준 색상**: 깃털 `#2A2A33`(검정), 부리·발 `#1A1A1A`, 표식 시 빛 `#FF8844`, 붉은 눈 `#FF3322`
- **특징**: 공중 선회하는 까마귀. 표식(mark/caw) 시 입 벌려 우는 정지 자세 + 주변 빛. 급강하(dive) 쪼기.
- **파일 목록**: `crow-{8방향}.png`(32×26, 비행) + `crow-mark.png`(울음·표식) + `crow-dive.png`(급강하)

```
Pixel art sprite sheet, transparent background.
A mutated CROW enemy for Zone 3 of LAGOMORPH roguelike (32x26 px, flying silhouette).
Black feathers (#2A2A33), dark beak and talons (#1A1A1A), glowing red eyes (#FF3322).
8-directional flying sprites in 2x4 grid (each 32x26 px), wings spread: [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames: [mark 32x26] beak open mid-caw, hovering, faint orange aura (#FF8844);
[dive 32x26] diving down, wings swept back, talons forward.
Top-down 45-degree view. 8-color limited palette.
```

---

## 5. 오소리 (Badger) — 잠행 돌격 탱커

- **게임 내 크기**: 44×34 px (낮고 다부진 체형)
- **기준 색상**: 몸 `#9A9488`(회갈), 얼굴 흑백 줄무늬 `#1A1A1A`/`#E8E8E0`, 발톱 `#C8CCD0`, 붉은 눈 `#FF3322`
- **특징**: 오소리 특유의 흑백 얼굴 줄무늬. 할퀴기(claw) 시 앞발 크게 휘두름. 땅파기(burrow) 시 흙더미 속으로 절반 들어간 모습.
- **파일 목록**: `badger-{8방향}.png`(44×34, 보행) + `badger-claw.png`(정면 할퀴기) + `badger-burrow.png`(땅파기 — 흙더미)

```
Pixel art sprite sheet, transparent background.
A mutated BADGER enemy for Zone 3 of LAGOMORPH roguelike (44x34 px, low sturdy body).
Gray-brown fur (#9A9488), iconic black-and-white facial stripes (#1A1A1A / #E8E8E0),
sharp digging claws (#C8CCD0), glowing red eyes (#FF3322).
8-directional walking sprites in 2x4 grid (each 44x34 px): [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames: [claw 44x34] front paws raised in a forward swipe (front-cone attack);
[burrow 44x34] half-submerged into a dirt mound, digging down.
Top-down 45-degree view. 10-color limited palette.
```

---

## 6. 사냥개 (Hound) — 5층 보스 "사냥개 무리" 구성원

- **게임 내 크기**: 42×34 px (1구역 여우보다 크고 사나운 체형)
- **기준 색상**: 털 `#3A3026`(어두운 갈), 입·이빨 `#E8E8E0`, 목줄 `#6B2A2A`(붉은 가죽), 붉은 눈 `#FF3322`
- **특징**: 사냥개 3마리가 무리 보스. 사나운 군집 추격견. 측면 lunge 시 도약하는 자세. 목줄(인간이 풀어놓은 사냥개) 디테일.
- **파일 목록**: `hound-{8방향}.png`(42×34, 추격) + `hound-idle.png` + `hound-lunge.png`(도약 돌진)

```
Pixel art sprite sheet, transparent background.
A HOUND enemy for Zone 3 of LAGOMORPH roguelike (42x34 px) — three of them form a pack boss.
Dark brown short fur (#3A3026), bared white teeth (#E8E8E0), a red leather collar (#6B2A2A),
glowing red eyes (#FF3322), lean aggressive hunting-dog build.
8-directional chase sprites in 2x4 grid (each 42x34 px): [N][NE][E][SE][S][SW][W][NW]
Plus 2 action frames: [idle 42x34] growling, low stance; [lunge 42x34] leaping forward in a pounce.
Top-down 45-degree view. 9-color limited palette.
```

---

## 7. 사냥꾼 보스 ("수석 사냥꾼", HunterBoss) — 10층 최종 보스

- **게임 내 크기**: 64×80 px (보스급 인간형, 일반 사냥꾼의 ~1.5배)
- **기준 색상**: 사냥복 `#5A4A30`, 망토/가죽 갑옷 `#3A2E1C`, 무기 금속 `#C8CCD0`, 광폭화 시 붉은 틴트 `#FF6666`, 붉은 눈 `#FF3322`
- **특징**: 활 + 단검을 모두 다루는 베테랑 사냥꾼. P1 조준 사격 + 올가미, P2 단검 콤보 + 사냥개 소환, P3 광역 화살. 위엄 있는 체구.
- **파일 목록**: `hunterboss-{8방향}.png`(64×80) + `hunterboss-aim.png`(활 조준) + `hunterboss-slash.png`(단검 콤보) + `hunterboss-summon.png`(사냥개 소환 호각) + `hunterboss-rage.png`(P3 광폭화)

```
Pixel art sprite sheet, transparent background.
THE HEAD HUNTER (HunterBoss), final boss of Zone 3 in LAGOMORPH roguelike (64x80 px).
A veteran human hunter — layered leather armor (#5A4A30), tattered cloak (#3A2E1C),
carries both a longbow and a steel dagger (#C8CCD0), faintly glowing red eyes (#FF3322),
imposing boss-tier presence (~1.5x a common hunter).
8-directional sprites in 2x4 grid (each 64x80 px): [N][NE][E][SE][S][SW][W][NW]
Plus 4 action frames on a strip:
[aim 64x80] longbow drawn, taking aim;
[slash 64x80] dagger combo lunge;
[summon 64x80] blowing a hunting horn to call hounds, arm raised;
[rage 64x80] Phase 3 enraged — body tinted red (#FF6666), eyes burning, wild stance.
Top-down 45-degree view. 12-color limited palette. Dwarfs common enemies.
```
...
- [ ] 7개 디렉토리 생성: `public/assets/enemies/{daggerhunter,bowhunter,snake,crow,badger,hound,hunterboss}/`
- [ ] 보조 텍스처 2개: `snare`(올가미 — 밧줄 고리 반투명), `hunter-arrow`(화살)
- [ ] 인간 사냥꾼은 **붉은 눈빛**으로 로봇 복선 유지 (구역 4 보스에서 로봇 정체 공개)
- [ ] PNG 배경 투명·안티에일리어싱 없음, 크기는 표시 크기 기준
- [ ] PNG 배치 완료 시 `BootScene` 의 구역 3 placeholder 매핑 제거 후 정식 로딩 패턴으로 교체
