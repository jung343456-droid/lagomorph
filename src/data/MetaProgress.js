 ...
 *   lagomorph_play_count      신규 런 시작 누적 횟수 (정수) — 이어하기(복원)는 제외
 *
 * 메타 코어 적립 규칙 (보존율 모델):
...
const PLAY_COUNT_KEY       = 'lagomorph_play_count'; // 신규 런 시작 누적 횟수 (이어하기 제외)
...
// ── 플레이 횟수 (신규 런 시작 누적) ──────────────────
//
// 신규 런을 시작할 때마다 1씩 증가한다(GameScene.create 의 !save 분기). 이어하기(복원)는 제외.
// 지금은 기록만 하고 소비처는 없음 — 이후 통계/도전과제/연출 등에서 사용 예정.

/** 누적 플레이 횟수 (정수). */
export function getPlayCount() {
  try {
    const v = parseInt(localStorage.getItem(PLAY_COUNT_KEY) || '0', 10);
    return Number.isFinite(v) && v >= 0 ? v : 0;
  } catch { return 0; }
}

/** 신규 런 시작 시 호출 — 플레이 횟수 +1. 증가 후 값 반환. */
export function incrementPlayCount() {
  const v = getPlayCount() + 1;
  try { localStorage.setItem(PLAY_COUNT_KEY, String(v)); } catch {}
  return v;
}
