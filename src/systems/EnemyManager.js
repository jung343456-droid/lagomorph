...
const ZONE34_HP_MULT   = 1.8; // HP ×1.8
const ZONE34_DMG_MULT  = 1.8; // 공격력 ×1.8
const ZONE34_SPD_MULT  = 1.1; // 이동속도 ×1.1 (speedMult / baseSpeedMult 경유)
const ZONE34_CORE_MULT = 1.5; // 코어 드롭 ×1.5 (적이 더 단단해진 만큼 보상 상향) — 상점 가격 배율(SHOP_ZONE_MULT)과 독립
...
export default class EnemyManager {
  constructor(scene, player) {
    ...
    this._poisoned   = new Map(); // Map<enemy, { ticksLeft, tickTimer }> — _tickStatus 처리
    this._burned     = new Map(); // Map<enemy, { ticksLeft, tickTimer }> — _tickStatus 처리
  ...
  update(delta) {
    ...
    // 독 데미지 틱 — 1초마다 정수 데미지 한 번 (max(2, floor(maxHp×1%))), ticksLeft 회 발생 후 해제.
    this._tickStatus(this._poisoned, dt, '#cc88ff', e => Math.max(2, Math.floor(e.maxHp * 0.01)));

    // 화상 데미지 틱 — 1초마다 정수 데미지 (max(4, floor(maxHp×2.5%))), ticksLeft 회 발생 후 해제.
    this._tickStatus(this._burned, dt, '#ff6622', e => Math.max(4, Math.floor(e.maxHp * 0.025)));
  ...
  spawnMidBoss(x, y) {
    ...
    if (zone >= 3) {
      if (df === 3) {
        ...
        // 표시 8층: 정예 사냥꾼 4기(단검×2 + 활×2) (spawnEnemy 가 구역 강화 + push 처리, 이후 엘리트 변이).
        // 정예지만 개별 elite-killed 드롭은 억제(_noEliteDrop) — 방 클리어(boss-cleared) 시
        // 다른 보스방처럼 패시브 아이템이 단 1개만 나오게 한다 (표시 5층 사냥개 무리와 동일).
        [['daggerhunter', -90], ['daggerhunter', -40], ['bowhunter', 40], ['bowhunter', 90]].forEach(([type, off]) => {
  ...
  /** 보스 자신의 로직에서 직접 생성한 소환 유닛(HunterBoss 의 Hound 등) 등록.
   *  spawnEnemy 는 ENEMY_CLASSES(일반 스폰 테이블)만 알고 있어 Hound 처럼 보스 전용 클래스는
   *  spawnEnemy('hound', ...)로 넘기면 Fox 로 폴백되는 문제가 있다 — _addBossUnit 을 그대로 재사용. */
  addSummonedUnit(unit, maxVel) {
    return this._addBossUnit(unit, isStrengthenedZone(this.floorNum), maxVel);
  }
  ...
  /**
   * DoT 상태이상(독·화상) 공용 틱 처리. entry = { ticksLeft, tickTimer }.
   * tickTimer 가 0 이하가 될 때마다 dmgFn(enemy) 만큼 피해 + ticksLeft 1 소모.
   * 지속시간 대신 남은 틱 수로 종료를 판정해 마지막 틱이 부동소수점 경쟁으로 유실되지 않는다.
   * (첫 틱은 부여 후 1초 뒤 — _applyPoison/_applyBurn 이 tickTimer 1 로 시작해 즉발을 막는다.)
   */
  _tickStatus(map, dt, dmgColor, dmgFn) {
    for (const [enemy, entry] of map) {
      if (!enemy.alive) { map.delete(enemy); continue; }
      entry.tickTimer -= dt;
      if (entry.tickTimer > 0) continue;
      entry.tickTimer += 1;
      entry.ticksLeft -= 1;

      const dmg  = dmgFn(enemy);
      const died = enemy.poisonHp(dmg);
      showDamageNumber(this.scene, enemy.x, enemy.y - enemy.gameObject.height / 2, dmg, dmgColor);
      if (died) {
        map.delete(enemy);
  ...
  _applyPoison(enemy) {
    ...
    // ticksLeft 10 → 1초 간격 10틱. tickTimer 1 → 첫 데미지는 부여 후 1초 뒤 발생 (즉발 X)
    this._poisoned.set(enemy, { ticksLeft: 10, tickTimer: 1 });
    enemy._hpFill?.setFillStyle(0xaa44ff);
  }

  _applyBurn(enemy) {
    if (this._burned.has(enemy)) return;
    // ticksLeft 3 → 1초 간격 3틱. tickTimer 1 → 첫 데미지는 부여 후 1초 뒤 발생 (즉발 X)
    this._burned.set(enemy, { ticksLeft: 3, tickTimer: 1 });
