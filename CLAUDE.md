...
│   ├── Snake.js             구역3 잠복 기습 + 물고 매달리기 (lurk/windup/strike/cling/retreat/stun), 매달림 중 이속×0.8(Player.applyBiteSlow)+지속데미지8dps(틱당4), 피격3회·5초 시 탈락
...
│   ├── PassiveItem.js      패시브 아이템 (ITEM_DEFS 33개), localStorage 해금
...
- **구역 2 (11~20층)**: `MIXED_POOL` — 구역 1 적 10종 혼합 풀(모든 층 공용). 스폰 시
  `_applyZoneBuff()`로 **HP ×1.8, 공격력 ×1.8, 이동속도 ×1.1, 코어 드롭 ×1.5** 강화(`ZONE34_*` 상수 — 네이밍은
...
- 출구방(`type='boss'`) 내용물 (`RoomManager._enterRoom`, **표시층 기준 분기**):
  ...
  - **중간 보스**(표시 3·8층) `spawnMidBoss`: 구역1·2 = Wolf×2(3)/BlackBear(8) / 구역3·4 = Hound×2(3)/정예 사냥꾼 4기(단검×2+활×2)(8)
