...
| B / X | 설치형 트랩 | 코어 소모 `max(1, 3-trapCostBonus)`, 쿨다운 0.5s, 피해 30, 크기 22px × `trapSizeMult`, 최대 5개 |
...
- **총 3층**, 각 층은 `DungeonGenerator`로 독립 생성된 8×6 격자 던전
- 층 구성: 일반 전투방 + 보스방 1개
- 적 수: 층 1 = 2~3, 층 2 = 3~4, 층 3 = 4~5
- 보스방: 층 1~2 → `spawnElite()` (Wolf + 수행원), 층 3 → `spawnBoss()` (Fang)
- 층 클리어(보스 처치) → 2.5초 후 `_advanceFloor()` → 다음 층 진입 (`floor < 3`)
- 층 3 클리어 → 구역 클리어 화면
- `RoomManager.floorNum`이 층 번호를 관리하고, `GameScene.currentFloor`와 동기화
