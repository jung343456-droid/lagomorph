...
│   ├── BowHunter.js         구역3 원거리 조준 + 올가미 덫 (idle/kite/aim/stun)
...
│   ├── Hound.js             구역3 표시5층 보스 "사냥개 무리" 구성원 3마리 (chase/windup/lunge/cooldown/stun)
...
- **총 40층 / 4구역** (`MAX_FLOOR=40`, `MAX_ZONE=4` — `constants.js`). `GameScene.currentFloor`는 1~40 선형
  ...
  - 보스/중간보스 분기는 **`displayFloor` 기준**(표시 5·10층=보스, 3·8층=중간보스) — `RoomManager._enterRoom`,
    `EnemyManager.spawnBoss/spawnMidBoss`. 구역3·4 표시5층=사냥개 무리(Hound×3), 표시10층=HunterBoss.
...
- 출구방(`type='boss'`) 내용물 (`RoomManager._enterRoom`, **표시층 기준 분기**):
  - **보스**(표시 5·10층) `spawnBoss`: 구역1·2 = FANG(5)/OWL KING(10) / 구역3·4 = 사냥개 무리 Hound×3(5)/HunterBoss(10)
